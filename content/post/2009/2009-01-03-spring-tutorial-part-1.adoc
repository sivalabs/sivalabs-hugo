...
Description: "Spring Tutorial Part 1"
Url: spring-tutorial-part-1
...
The Spring Framework is an Application Framework fro developing Java based applications starting from stand alone 
applications to SOA style applications. Let us see how we can use Spring to develop a simple stand alone application 
using Spring.  
  
The requirement is to develop a UserService component which takes the credentials of the user and authenticate 
the credentials against the values stored in the database. The UserService component makes use of EntityManager, 
which takes care of all the database operations. The EntityManager in turn uses the UserDAO to perform 
the user login operation.  
